...
comptes_communes_2000_2017 <-
  comptes_communes_2000_2017 %>% mutate(
    ...
    cotis_fonciere_enreprises_base = as.numeric(cotis_fonciere_enreprises_base),
...
comptes_communes_martinique_groupes <-
  comptes_communes_martinique %>% group_by(annee, strate) %>% mutate(
    ...
    charges_personnel_moyennes = mean(as.numeric(charges_personnel))
  )
comptes_communes_martinique_groupes <- comptes_communes_martinique_groupes %>% mutate(depenses_equipements_moyennes = mean(invest_empl_equipements))
...
```{r}
comptes_communes_martinique_groupes %>%
ggplot(aes(annee, depenses_equipements_moyennes)) +
geom_point(alpha = 0.5) +
geom_smooth(method = "lm") +
facet_wrap( ~ strate)
```
